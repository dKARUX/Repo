...
triggerBeat = (event) => {
    ...
    if(keyboardKeyCode in beats){
        let beatsKeyRead = beats[keyboardKeyCode];
        beatsKeyRead.beat.playAudio();
        beatsKeyRead.button.select();
        
    } else {
        console.log("Error: Please choose a valid option based on what is displayed in the interface...");
    }
}
